...
for (i in 1:n) { 
  ...
  data_meteo <- as.data.frame(data_meteo)
  data_meteo <- select(data_meteo,colnames(data_meteo)[1], colnames(data_meteo)[2], colnames(data_meteo)[3], colnames(data_meteo)[8], colnames(data_meteo)[9], colnames(data_meteo)[10], colnames(data_meteo)[11], colnames(data_meteo)[12], colnames(data_meteo)[13])
  data_meteo$`PRECIPITAÇÃO TOTAL, HORÁRIO (mm)` <- as.numeric(data_meteo$`PRECIPITAÇÃO TOTAL, HORÁRIO (mm)` )
  ...
  data_meteo <- select(data_meteo, colnames(data_meteo)[1],colnames(data_meteo)[3],colnames(data_meteo)[10]) #I delete the previous temp columns
  
  #I calculate the daily average of temperatures and the sum of the daily precipitation. 
  
  colnames(data_meteo)[2] <- 'Precipitation'
